...
const removeNulls = (obj: any): any => {
  ...
  if (typeof obj === "object") {
    return Object.fromEntries(
      Object.entries(obj).map(([key, value]) => [key, removeNulls(value)])
    );
  }
  return obj;
};
...
export const prepareFullPlan = action({
  args: {
    planId: v.string(),
  },
  handler: async (ctx, { planId }) => {
    try {
      ...
      const { activityPreferences, companion, fromDate, toDate, budgetTier } = planMetadata || {};
      ...
      const nameMsg = completion?.choices[0]?.message?.content as string;
      const modelName = removeNulls(JSON.parse(cleanJsonString(nameMsg))) as any;

      const safeTopPlaces = (modelName.topplacestovisit || []).map((place: any) => ({
        ...place,
        coordinates: {
          lat: Number(place.coordinates?.lat || 0),
          lng: Number(place.coordinates?.lng || 0),
        },
      }));
      ...
      console.error(`Error occurred in prepareFullPlan: ${error}`);
      const emptyPlan = await fetchEmptyPlan(ctx, planId);
      if (emptyPlan) {
         await ctx.runMutation(internal.plan.updateGenerationError, {
           planId: emptyPlan._id,
         });
      }
    }
...
export const prepareBatch1 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const modelName = removeNulls(JSON.parse(cleanJsonString(nameMsg))) as Pick<
        Doc<"plan">,
        "abouttheplace" | "besttimetovisit" | "tripHighlights" | "weatherAnalysis"
      >;
...
export const prepareBatch3 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const safeTopPlaces = (modelName.topplacestovisit || []).map((place) => ({
        ...place,
        coordinates: {
          lat: Number(place.coordinates?.lat || 0),
          lng: Number(place.coordinates?.lng || 0),
  ...
  args: {
    ...
    itinerary: v.array(
      v.object({
        ...
        foodRecommendations: v.optional(
          v.object({
        ...
        quickBookings: v.optional(v.array(v.string())),
...
export const updateFullPlan = internalMutation({
  args: {
    planId: v.id("plan"),
    abouttheplace: v.string(),
    besttimetovisit: v.string(),
  ...
  handler: async (ctx, args) => {
    const plan = await ctx.db.get(args.planId);
    console.log(`updateFullPlan called on planId : ${args.planId}`);
    await ctx.db.patch(args.planId, {
      abouttheplace: args.abouttheplace,
      besttimetovisit: args.besttimetovisit,
      tripHighlights: args.tripHighlights,
...
export const updateGenerationError = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const newPlan = await ctx.db.insert("plan", {
      ...
      contentGenerationState: {
        ...
        tripHighlights: state,
        weatherAnalysis: state,
        budgetRange: state,
      },
    });
...
export const updateSingleDayItinerary = internalMutation({
  args: {
    planId: v.id("plan"),
    dayIndex: v.number(),
    dayData: v.any(),
  ...
  handler: async (ctx, args) => {
    ...
    newItinerary[args.dayIndex] = args.dayData;
    await ctx.db.patch(args.planId, {
...
export const regenerateSingleDay = action({
  args: {
    planId: v.string(),
    dayIndex: v.number(),
  },
  handler: async (ctx, { planId, dayIndex }) => {
    try {
      const emptyPlan = await fetchEmptyPlan(ctx, planId);
      if (!emptyPlan || !emptyPlan.itinerary) return null;
      ...
      const completion = await generateSingleDayItinerary({
        userPrompt: emptyPlan.userPrompt,
        activityPreferences,
        companion,
        fromDate,
      ...
      const nameMsg = completion?.choices[0]?.message?.content as string;
      const modelName = removeNulls(JSON.parse(cleanJsonString(nameMsg)));

      await ctx.runMutation(internal.plan.updateSingleDayItinerary, {
        planId: emptyPlan._id,
        dayIndex,
        dayData: modelName,
      });
      ...
      console.error(error);
      throw new Error(`Error occurred in regenerating single day: ${error}`);
    }
  },
});
