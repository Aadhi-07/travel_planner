...
export default function CommunityPlans({
  ...
  const filteredResults = useMemo(() => {
    if (!results) return [];
    if (!locationFilter) return results;
    return results.filter((plan) =>
      plan?.nameoftheplace?.toLowerCase().includes(locationFilter.toLowerCase())
    );
  }, [results, locationFilter]);

  const noPlansToShow =
    (!filteredResults || filteredResults.length === 0) && status === "Exhausted";
    ...
    <SidebarProvider>
      ...
      <SidebarInset className="flex flex-col">
        ...
        <div className="flex flex-col gap-2 pt-0">
          ...
          {status === "CanLoadMore" && (
            <Button
              className="w-fit rounded-full mx-auto bg-[#c86d51] text-white 
              hover:bg-[#b55c41] dark:bg-background dark:text-foreground dark:border-2 dark:border-border hover:scale-105 transition-all duration-100"
              onClick={() => loadMore(8)}
            >
              Load More
            </Button>
